...
df.mfcc = data.frame(gender=character(),
...
for (i in 1:length(file_list)){
  ...
  gender = data$sex[data$filename == file]
  
  gen.mat = matrix(gender)
  colnames(gen.mat) = "gender"
  
  val.mat = matrix(c(mean.vec, sd.vec), nrow=1)
  colnames(val.mat) = colnames(df.mfcc)[2:dim(df.mfcc)[2]]
  
  row = merge(gen.mat, val.mat)
...
setwd("~/STAT479/archive")
write.csv(df.mfcc.no, "mfcc_gender.csv", row.names = F)